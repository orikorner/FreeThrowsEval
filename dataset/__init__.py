...
def get_dataloader(subset, config, batch_size=16, num_workers=4, shuffle=True):
    ...
    if config.objective_mode == 'trj' and subset == 'train':
        cls_dataset = BBFTSDataset(subset='train', config=config)
        trj_dataset = BBFTSDataset(subset='extras', config=config)
        ...
        cls_dataset = BBFTSDataset(subset=subset, config=config)
        ...
        if subset == 'train_full':
            ...
            assert subset == 'test'
            cls_dataset = BBFTSDataset(subset='test', config=config)
